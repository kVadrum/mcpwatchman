    ...
    JSON.stringify(
      {
        ...
        unmeasured_faults_note:
          "On an axis that DOES carry a score, `unmeasured_faults` lists whose gap the unmeasured remainder is — the sub-checks that could not be evaluated and were renormalised out before the score was computed. It is usually empty, which means the gap needs no separate explanation. It never contains a gap caused by our own run: a partly-measured axis whose missing part is our fault is refused at publication exactly as a wholly unmeasured one is, so a scored axis here is never hiding a broken scanner. Read it together with `assessed_weight`, which says how much of the axis the score covers.",
        coverage_note:
