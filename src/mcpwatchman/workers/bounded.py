...
**This is why the symptom reads as ambient flakiness and is not.** `CLAUDE.md`
records that a scan's failure rate tracks concurrent load, and that is true —
but the load here was OURS, generated by the scan itself, one leaked process at
a time. A quiet box does not fix it; it only delays the first timeout.
